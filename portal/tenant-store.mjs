...
for(const key of ['companies','shipments','ncts'])if(body[key]!==undefined){if(!Array.isArray(body[key]))throw Error('Invalid data');if(key==='companies'){for(const account of state.users){const previous=state.companies.find(c=>String(c.id||c.name)===account.companyId);const replacement=previous&&body[key].find(c=>c.name===previous.name);if(replacement)account.companyId=String(replacement.id||replacement.name);}}if(key==='companies'){const ids=new Set(),names=new Set();for(const c of body[key]){if(c.isCarrier&&(!(c.serviceCountries||[]).length||!(c.contacts||[]).some(x=>['Hava','Kara','Deniz','Demiryolu'].includes(x.transportMode)&&/^\S+@\S+\.\S+$/.test(x.email||''))))throw Error('Acente hizmet ülkesi ve birim e-postası gerekli');const id=String(c.id||c.name),name=String(c.name||'').trim().toLocaleLowerCase('tr');if(!id||!name||ids.has(id)||names.has(name))throw Error('Firma kimliği ve unvanı benzersiz olmalı');ids.add(id);names.add(name);}}state[key]=body[key];if(key!=='companies')state[key]=state[key].map(r=>bind(r,key));}

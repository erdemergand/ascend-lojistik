...
test('server session isolates company data and document URLs, including forged browser roles',async()=>{
 ...
 async function call(path,body,cookie){if(path==='/api/tenant/sync'&&body&&body._revision===undefined){const snapshot=await (await call('/api/tenant/data',null,cookie)).json();body={...body,_revision:snapshot.revision};}return fetch(base+path,{method:body?'POST':'GET',headers:{Authorization:auth,...(body?{'Content-Type':'application/json'}:{}),...(cookie?{Cookie:cookie}:{})},body:body?JSON.stringify(body):undefined});}
 try{

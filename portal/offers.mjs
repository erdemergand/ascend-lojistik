...
export function offerService(store) {
 ...
 for(const key of ['purchase','sale'])if(!(key==='sale'&&(b[key]===''||b[key]==null))&&(!/^\d+(\.\d{1,2})?$/.test(String(b[key]))||!Number.isSafeInteger(Math.round(Number(b[key])*100))))throw Error('Tutarlar en fazla iki ondalık basamakla girilmeli');
 ...
 const body={carrierId:b.carrierId,purchase:Math.round(Number(b.purchase)*100)/100,sale:b.sale===''||b.sale==null?null:Math.round(Number(b.sale)*100)/100,currency:b.currency,validUntil:b.validUntil,status:b.status,transit:String(b.transit||'').slice(0,300),costs:String(b.costs||'').slice(0,2000),notes:String(b.notes||'').slice(0,3000)};
 ...
 send(u,b){admin(u);if(b.reviewed!==true)throw Error('Maili kontrol ettiğinizi onaylayın');const row=db.prepare('SELECT * FROM offers WHERE id=?').get(b.id);if(!row||row.version!==b.version)throw conflict();if(JSON.parse(row.body).sale==null)throw Error('Önce müşteriye sunulacak navlun tutarını kaydedin');
